---
name: develop
description: "j-voca 기능 개발 전체 파이프라인을 자동 실행하는 오케스트레이터. Jira 이슈번호 또는 기능 설명을 받아 요구사항 분석 → 계획 수립 → 사용자 승인 → 구현 → 빌드/테스트 → 코드리뷰 → main 직접 머지까지 수행한다. '개발해줘', '구현해줘', '기능 추가', 'JVOCA-숫자', 이슈번호를 언급하면 반드시 이 스킬을 사용할 것."
---
...
### Phase 6: main 직접 머지 + 기록

이 프로젝트는 개인 앱이므로 **PR을 만들지 않는다.** 리뷰 PASS 후 feature 브랜치를 main에 직접 머지한다.

1. `git checkout main && git pull` 로 main을 최신화한다
2. `git merge feature/...` 로 머지한다 (fast-forward 가능하면 그대로, 아니면 머지 커밋)
3. `git push origin main` (main push 시 GitHub Pages 배포가 자동 실행된다)
4. 머지된 feature 브랜치는 로컬/원격에서 삭제한다
5. Jira 이슈가 있는 경우, 머지 커밋 해시를 Jira에 댓글로 기록한다
6. 사용자에게 머지 커밋 해시와 변경 요약을 보고한다
...
| main 머지 충돌 | 충돌 파일을 사용자에게 알리고 해결 방법을 제안한다. 임의로 해결하지 않는다 |
...
7. Phase 6: main 직접 머지 → Jira에 머지 커밋 기록
