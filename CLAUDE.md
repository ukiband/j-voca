...
- 예문은 words.json 과 분리된 `public/data/sentences.json`(Dexie `sentences` 테이블, PK `[wordId+date]`)에 두고 앱은 읽기만 한다. `.github/workflows/generate-sentences.yml`이 매일 KST 10시에 `scripts/generate-sentences.mjs`로 최신 레슨(step ≥ 2) 단어에 단어당 최대 7개까지 Gemini 로 생성해 커밋한다
