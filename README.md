...
6. 활용형을 외우려면 전체 복습 아래의 **동사 활용**을 선택합니다. て형·ない형·た형·가능형 중 원하는 형태를 체크하고, 사전형을 보고 변환한 뒤 정답과 규칙을 확인합니다. `한 번 더`는 이번 연습 끝에서 다시 출제하며, 학습 기록에는 반영하지 않습니다.
...
- **백업·설정**: JSON으로 학습 기록을 내보내고 가져올 수 있습니다. 화면 테마(시스템·라이트·다크), 글자 크기 4단계와 학습 기록 초기화를 지원합니다. 테마 기본값은 시스템이며, 선택은 해당 브라우저에 저장됩니다.
...
동사 활용 기능의 범위와 설계 결정은 [논의 메모](docs/verb-forms-discussion.md)에 정리했습니다.
...
테스트는 FSRS 복습 계산, Step·레슨 및 단어 필터링, Gemini 응답 처리, 예문 유효성, 동사 활용 규칙·출제 큐·분류 마이그레이션, 실제 JSON 데이터의 정합성 등을 검증합니다.

### 동사 분류 마이그레이션

`words.json`의 동사에는 `verbGroup`(1·2·3류 또는 미확인 `null`), `isDictionaryForm`(등록 표기가 사전형인지), `potentialAllowed`(가능형 연습 대상인지)를 저장합니다. 기존 표기·읽기를 활용하며 별도 사전형이나 형태별 정답은 저장하지 않습니다. 사진 추출 시 분류도 함께 준비하고, 저장 전 확인 화면과 기존 단어 수정 화면에서 조정할 수 있습니다. 표기·읽기를 수정하면 분류를 다시 확인하도록 초기화합니다.

2026-09-21에 검토한 기존 동사 173개 중 143개가 사전형이고, 30개는 활용형·어간 등이어서 연습에서 제외합니다. 같은 표기·읽기·분류의 중복을 합친 출제 대상은 121개이며, 네 형태를 모두 고르면 가능형 제외 항목을 반영해 470문제입니다. 원래 단어와 FSRS 데이터는 유지합니다.

검토한 항목만 다시 적용하려면 다음 명령을 실행합니다. 재실행해도 결과는 같으며, 검토 후 표기·읽기가 달라진 항목이 있으면 저장 전에 중단합니다.

```bash
node scripts/migrate-verb-metadata.mjs
```

활용 규칙은 [도쿄외국어대학 활용표](https://www.coelang.tufs.ac.jp/mt/ja/gmod/contents/card/038.html)와 [MLC 가능형 자료](https://www.mlcjapanese.co.jp/Download/Potential_Form.pdf)를 참고했습니다. 가능형은 표준형(예: `食べられる`)을 사용하고, 문맥 없이 연습하기 부적절한 조합은 분류 정보로 제외합니다.
