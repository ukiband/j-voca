...
## 예문 규칙
- 등록된 뜻(meaning)과 읽기(reading)에 맞는 쓰임으로, 초급 교재 수준의 짧고 쉬운 완결된 문장을 만듭니다. 20자 안팎을 권장하되 자연스러운 문법과 정확한 뜻을 우선합니다.
...
- 목표 단어 자체를 사용합니다. 다른 단어나 파생 명사로 대체하지 않습니다 (예: 釣る → 魚を釣ります, 釣りをします로 대체하지 않음). 동사 어간·접속형은 올바른 활용 안에서 사용하고, 조사·자동사/타동사·명사와 동사의 결합을 확인합니다.
- 여러 뜻이 있는 단어도 입력에 등록된 뜻으로만 사용합니다. 예를 들어 書いて의 뜻이 '쓰고 나서'라면 뒤에 이어지는 행동을 쓰고, '써 주세요'라는 요청으로 바꾸지 않습니다.
- 입력 표기·읽기·뜻이 모순되어 올바른 예문을 만들 수 없는 단어는 결과에서 생략합니다. 오류를 그대로 복제하거나 억지 문장을 만들지 않습니다.
...
- sentence와 reading을 앞에서부터 대조해 한자·숫자 읽기와 띄어쓰기 외에는 바뀐 부분이 없는지 확인합니다. 특히 가타카나, 조사, 활용 어미와 [[ ]] 위치가 같아야 합니다.
...
입력: {"wordId":100001,"word":"弾く","reading":"ひく","meaning":"(악기를) 치다","pos":"동사","existing":[]}
